(function ($) {
    ...
    HT.removeModel = () => {
        $(document).on("click", ".deleted", function () {
            let _this = $(this);
            let modelId = _this.parents(".search-result-item").data("modelid");
            console.log("Model bị xóa:", modelId); // In model bị xóa
        ...
        if (data.length) {
            for (let i = 0; i < data.length; i++) {
                ...
                html += `<button 
                            ...
                            data-flag="${flag}" 
                            data-canonical="${data[i].canonical}" data-image="${data[i].image}" 
