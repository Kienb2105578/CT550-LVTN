(function ($) {
    ...
    HT.updateVariantHtml = (variantData) => {
        initialQuantity = variantData.variant_quantity;
    ...
    let initialNewQuantity = null;

    HT.saveVariantUpdate = () => {
        $(document).on("click", ".saveUpdateVariant", function () {
            console.log("Số lượng ban đầu:", initialQuantity);
            initialNewQuantity = $("input[name=variant_quantity]").val();
            console.log("Số lượng sau khi thay đổi:", initialNewQuantity);
            let variant = {
    ...
    let totalQuantity = 0;
    HT.calculateTotalQuantity = () => {
        // Lấy số lượng cũ từ input
        let currentProductQuantity = parseInt($("#productQuantity").val()) || 0;
        console.log(currentProductQuantity);

        // Kiểm tra và ép kiểu nếu cần
        initialQuantity = parseInt(initialQuantity) || 0; // Ép kiểu về số, nếu null hoặc không phải số thì thành 0
        initialNewQuantity = parseInt(initialNewQuantity) || 0; // Ép kiểu về số, nếu null hoặc không phải số thì thành 0
        totalQuantity = 0; // Khởi tạo biến tổng số lượng

        // Nếu có sự thay đổi trong số lượng ban đầu và mới, tính lại
        if (initialNewQuantity !== null && initialQuantity !== null) {
            totalQuantity =
                totalQuantity - initialQuantity + initialNewQuantity;
        } else {
            $(".variant-row").each(function (index, value) {
                // Lặp qua mỗi dòng trong bảng
                let _this = $(this); // Lấy phần tử dòng hiện tại
                let variantKey = Array.from(_this[0].classList)
                    .find((cls) => cls.trim().startsWith("tr-variant-"))
                    .split("variant-")[1]
                    .trim();
                ...
                let dataIndex = variant.sku.findIndex((sku) =>
                    sku.includes(variantKey)
                ); // Tìm chỉ số của biến thể trong mảng sku

                if (dataIndex !== -1) {
                    // Nếu tìm thấy biến thể
                    let quantityValue =
                        parseInt(variant.quantity[dataIndex]) || 0; // Chuyển giá trị thành số nguyên hợp lệ
                    // Cộng số lượng vào tổng
        ...
        console.log(
            "Tổng số lượng tất cả các biến thể:",
            parseInt(totalQuantity)
        );

        // Kiểm tra nếu totalQuantity khác với giá trị hiện tại của productQuantity
        if (totalQuantity !== currentProductQuantity && totalQuantity !== 0) {
            // Nếu có sự thay đổi, gọi AJAX để cập nhật số lượng trong input mà không thay đổi CSDL
            $.ajax({
                url: "/ajax/product/updateProductQuantity",
                type: "post",
                data: {
                ...
                success: function (response) {
                    $("#productQuantity").val(response.updated_quantity);
                },
                error: function () {
