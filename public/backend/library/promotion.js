(function ($) {
        ...
        $(document).on("click", ".btn-js-100", function () {
            ...
            $discountTd.append(
                $("<div>", { class: "uk-flex uk-flex-middle" })
                    .append(
                        $("<input>", {
                            type: "text",
                            name: "order_amount_range[amountValue][]",
                    ...
                    .append(
                        $("<select>", {
                            ...
                            .attr("name", "order_amount_range[amountType][]")
            ...
            let deleteButton = $("<td>").append(
                $("<div>", {
                    ...
                    '<svg data-icon="TrashSolidLarge" aria-hidden="true" focusable="false" width="15" height="16" viewBox="0 0 15 16" class="bem-Svg" style="display: block;"><path fill="currentColor" d="M2 14a1 1 0 001 1h9a1 1 0 001-1V6H2v8zM13 2h-3a1 1 0 01-1-1H6a1 1 0 01-1 1H1v2h13V2h-1z"></path></svg>'
                )
            );
                ...
                <td class="discountType">
                    <div class="uk-flex uk-flex-middle">
                        <input 
                            ...
                            value="${$amountValue}"
                        >
                        <select name="order_amount_range[amountType][]" class="multipleSelect2" id="">
                            <option value="cash" ${
    ...
    HT.chooseProductPromotion = () => {
        $(document).on("click", ".search-object-item", function (e) {
            ...
            // Đợi trạng thái checkbox cập nhật xong
            setTimeout(() => {
                ...
                if (isChecked) {
                    // Xóa sản phẩm đã chọn (cả product_id và product_variant_id)
                    objectChoose = objectChoose.filter(
                        (item) =>
                            !(
                                item.product_id === objectItem.product_id &&
                                item.product_variant_id ===
                                    objectItem.product_variant_id
                            )
                    );
    ...
    HT.confirmProductPromotion = () => {
        ...
        if (preloadObject.id && preloadObject.id.length > 0) {
            objectArray = preloadObject.id.map((id, index) => ({
                product_id: id || "null",
                product_variant_id:
                    preloadObject.product_variant_id[index] || "null",
