(function ($) {
    ...
    HT.voiceSearch = () => {
        if (
            ...
            HT.recognition = new SpeechRecognition();

            HT.recognition.lang = "vi-VN";
            HT.recognition.continuous = false; // Chỉ nhận diện 1 lần
            HT.recognition.interimResults = false;

            HT.isRecognizing = false;
            ...
            $("#voice-search").on("click", function () {
                if (HT.isRecognizing) {
                    ...
                    HT.recognition.start();
                    $(this)
                        .removeClass("fa-microphone")
                        .addClass("fa-microphone-lines");
                    HT.isRecognizing = true;
                }
            });

            HT.recognition.onresult = function (event) {
                const transcript = event.results[0][0].transcript;
                $('input[name="keyword"]').val(transcript);
            };

            HT.recognition.onerror = function (event) {
