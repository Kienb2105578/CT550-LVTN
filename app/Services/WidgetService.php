    ...
    private function widgetAgrument($widget, $language, $param)
    {
        ...
        if (strpos($widget->model, 'Catalogue')) {
            ...
            if (isset($param['object'])) {
                $relation[$model] = function ($query) use ($param) {
            ...
            $relation[$model] = function ($query) {
                // Xóa phần liên quan đến languages ở đây
            };
        }
        return [
            'condition' => [
            ...
            'flag' => true,
            'relation' => $relation, // Trả về quan hệ đã cập nhật
            'param' => [
